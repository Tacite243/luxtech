...
const navLinks = [
  ...
  // Contact est ajouté pour le desktop via le spread plus bas
];
...
export default function Navbar() {
  ...
  return (
    <>
      {/* -------- NAVBAR DESKTOP (corrigée pour s'afficher) -------- */}
      <header
        className={`hidden md:block md:fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${scrolled ? "bg-[#111827]/90 backdrop-blur-sm shadow-lg" : "bg-transparent"
          }`}
      >
        <nav className="container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-8">
            {[...navLinks, { name: "Contact", href: "/contact" }].map((link) => {
              ...
              const className = link.name === "Boutique" ? storeLink : normalLink;

              return (
                <Link key={link.name} href={link.href} className={className}>
                  {link.name}
                  {link.name !== "Boutique" && isActive && (
                    <motion.span
                      layoutId="underline"
          ...
          {navLinks.map((link) => {
            ...
            return (
              <li key={`bottom-nav-${link.name}`}>
                <Link
                  href={link.href}
                  className={`flex flex-col items-center text-xs font-medium transition-all duration-300 ${isActive ? "text-[#FBBF24] scale-105" : "text-gray-400 hover:text-white"
                    }`}
                >
                  <Icon size={22} />
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            >
              {[...navLinks, { name: "Panier", href: "/cart", icon: ShoppingCart }, { name: "Contact", href: "/contact", icon: Phone }].map(
                (link) => (
                  <motion.li
