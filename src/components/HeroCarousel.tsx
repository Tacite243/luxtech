  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-[#000000]">
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center text-center text-[#FFFFFF] px-4">
        ...
        >
          Notre entreprise allie expertise technique et créativité pour offrir
          des services complets en construction, électricité et design
          d&apos;intérieur.
        </motion.p>
        <motion.button
          initial={{ opacity: 0, y: 20 }}
      ...
      <button
        onClick={() => paginate(-1)}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-20 rounded-full bg-white/20 p-2 hover:bg-white/40"
      >
        <FiChevronLeft size={24} className="text-white" />
      </button>
      <button
        onClick={() => paginate(1)}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-20 rounded-full bg-white/20 p-2 hover:bg-white/40"
      >
