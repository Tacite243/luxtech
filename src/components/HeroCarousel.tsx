// Le nom du fichier est probablement HeroSection.tsx
"use client";

import { useState, useEffect } from 'react';
import { motion, AnimatePresence, Variants } from 'framer-motion';
import { FiChevronLeft, FiChevronRight } from 'react-icons/fi';
// Importez le composant Image de Next.js pour l'optimisation
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-primary">
      <AnimatePresence initial={false} custom={direction}>
        {/* CHANGEMENT ICI : Le motion.div devient un conteneur simple */}
        <motion.div
          key={page}
          custom={direction}
        ...
        >
          {/* On utilise le composant Image de Next.js pour couvrir la zone */}
          <Image
            src={images[imageIndex]}
            alt="Image de fond pour la section principale"
            fill // Remplit le conteneur parent
            style={{ objectFit: 'cover' }} // Équivalent de bg-cover
            priority={true} // Charge la première image plus rapidement
          />
        </motion.div>
      </AnimatePresence>
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center text-center text-white px-4">
        ...
        >
          L'Innovation au Coeur de Votre 
          <br />
          <span className="text-secondary">Transformation Numérique</span>
        </motion.h1>
        ...
        <motion.div 
          ...
          className="w-24 h-1 bg-accent-yellow my-6"
        />
        <motion.p 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          Nous concevons des solutions technologiques robustes et élégantes qui propulsent votre entreprise vers l'avenir.
        </motion.p>
        <motion.button 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
